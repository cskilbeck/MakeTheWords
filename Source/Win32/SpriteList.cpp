...
void DrawRectangle(SpriteList &spriteList, Vec2 const &topLeft, Vec2 const &size, Color color)
{
	...
	if(size.x > 2)
	{
		spriteList.AddSprite(topLeft + Vec2(1,0), Vec2(size.x - 2, 1), color);
		spriteList.AddSprite(topLeft + Vec2(1,size.y), Vec2(size.x - 2, 1), color);
	}
	if(size.y > 1)
	{
